...
46. 在携带 `modulePages / dashboard_metrics` 新拦截的新版本上再次重试 `c6f...` 后，任务再次真实穿过 `build` 并进入 `verify_run`，说明 `ColdchainPage.tsx` 与 `Dashboard.tsx` 的上一轮单点已被绕开；新的远端 `tsc -b` 失败点进一步收敛为 `IncidentIdPage.tsx` 误访问 `APP_PROFILE.roles`，触发 `TS2339: Property 'roles' does not exist on type 'AppProfile'`。真实画像里只有 `user_roles`，没有 `roles`。现已把 `APP_PROFILE.roles` 纳入模块页非法画像字段拦截，并补充与线上 `IncidentIdPage.tsx` 同形态的回归测试；后续这类页面会在 `build` 阶段直接回退到结构化模块模板，而不是拖到 `verify_run` 才失败。
47. 在携带 `APP_PROFILE.roles` 新拦截的版本上再次重试 `c6f...` 后，这一轮没有再掉进新的 `verify_run` 单点，而是卡在 `build` 的 `core_invalid_retry`：线上 timeline 显示 `App.tsx` 单文件重试连续两次命中 `LLM JSON parse error (deepseek-v4-pro): Unterminated string ... frontend/src/App.tsx`，说明 `core_invalid_retry` 仍会像此前的 `module_invalid_retry` 一样被长 JSON 响应拖住。复核本地代码后确认，当前逻辑只在所有 core 重试结束后才统一走 `core_structural_fallback`，不会在单文件 retry 的 parse error / files payload 缺失时立即接管。现已把 `core_invalid_retry` 补成与模块页一致的即时 fallback 路径：当 `App.tsx / Login.tsx / Dashboard.tsx` 的 retry 响应解析失败或缺少 `files` 负载时，直接对当前 chunk 应用结构化 core 模板并重新校验，同时补充 `App.tsx` 重试解析失败的回归测试；后续这类 core 单文件长响应不会再把整轮 `build` 长时间拖在 retry 中。
