...
from app.services.document.manual_compose import build_variation_seed
...
"""
    ...
    async def generate_app_code(
        ...
        is_monitor_module_retry = retry_module_path.endswith("MonitorPage.tsx")
        is_settings_module_retry = retry_module_path.endswith("SettingsPage.tsx")
...
46. 若 `module_pages` 提供 `page_design_brief`，必须逐页落实该 brief 中的 layout_pattern / visual_accent，禁止与同任务其他模块页复用相同分区顺序与组件组合；`module_retry_attempt` 递增时须进一步拉开布局差异。
...
43. 即便这是首轮 `App.tsx` 生成，也要优先采用精简路由壳与轻量导航，不要输出超长导航配置、复杂装饰层级或大段静态说明；但不同任务的壳层仍需主动拉开差异，不要都收敛成“纯色顶栏 + 一行文字导航 + 下方单一内容画布”。
44. App.tsx 不要再写 `useMemo`、`useLocation`、`useNavigate`、`NavLink`、`Outlet`、`TabsProps`、`Tabs`，也不要导入 `Row, Col, Card, Button, Space` 这一整套整页装饰性组件；请压缩为更短的顶层路由壳，但可以在 `header/nav + Routes`、`分段导航 + Routes`、`顶部索引条 + 内容画布 + Routes` 之间结合当前任务自主选择，禁止把所有项目固定成同一种壳层。
45. 若上一版 `App.tsx` 预览中出现旧后台壳层、演示式文案、无效导入，或一上来就是 `import React, { useMemo } from 'react'` / `import { Routes, Route, Navigate, useLocation, useNavigate } from 'react-router-dom'` / `import { Routes, Route, Navigate, Link } from 'react-router-dom'` / `import { Tabs } from 'antd'` / `import type { TabsProps } from 'antd'` / `import { Row, Col, Card, Button, Space } from 'antd'` 这类重型顶层壳，那么这一轮必须整体替换该模式，而不是局部修补。
"""
...
"""
        ...
        if is_single_module_retry and retry_module_page_variant == "workspace":
            system_prompt += (
                "59. 当前模块 page_variant=workspace：请体现真实工作区特征，并围绕当前主题在以下结构中至少择一重构：`标题区 + 任务带 + 主结果区`、`标题区 + 摘要带 + 双区块列表`、`标题区 + 主表格/列表 + 右侧辅助信息区`；"
                " 不要所有任务都收敛成同一套“检索输入 + 主按钮 + 单表格”骨架，也不要生成抽屉、侧栏、复杂卡片矩阵或长篇说明。"
                " 也不要回退成 `import React, { useState } from 'react'` + `import { Input, Button, Card, Tag, Statistic } from 'antd'`"
                " + `import { SearchOutlined, PlusOutlined } from '@ant-design/icons'` + `const [search, setSearch] = useState('')`"
                " 这一类重型 antd 工作区模板；更不要回退成"
                ...
                " 这种蓝底白卡居中壳。"
                " 也不要回退成 `import React from 'react'` + `import { Input, Button, Tag } from 'antd'`"
                " + `import { SearchOutlined, PlusOutlined } from '@ant-design/icons'`"
                " + `<div style={{ padding: 24, background: '#f0fdf9', minHeight: '100vh' }}>`"
                " 这种绿色通用工作区壳。"
                " 也不要回退成 `import { useState } from 'react'` + `import { Card, Input, Button, Tag } from 'antd'`"
                " + `const [search, setSearch] = useState('')` + `SearchOutlined/PlusOutlined`"
                " 这种带 Card 的泛化工作区模板。"
                " 也不要回退成 `import { useState } from 'react'` + `import { Card, Row, Col, Statistic, Input, Button, Space, Tag, Table } from 'antd'`"
                " + `const [searchText, setSearchText] = useState('')` + `SearchOutlined/PlusOutlined`"
                " 这种统计卡矩阵 + 检索框的通用工作区模板。"
                " 若当前模块其实是 OrdersPage，也不要回退成 `import React from 'react'` + `import { Input, Button, Card, Space, Typography } from 'antd'`"
        ...
        if is_single_module_retry and retry_module_page_variant == "insight":
            system_prompt += (
                "60. 当前模块 page_variant=insight：请体现洞察页特征，并围绕当前主题在以下结构中至少择一重构：`标题区 + 2~3 个业务摘要 + 结果表`、`标题区 + 洞察摘要 + 左右并列信息区`、`标题区 + 过滤条 + 摘要列表 + 结果区`；"
                " 不要所有任务都收敛成同一套“2~3 个摘要块 + 检索输入 + 单表格”骨架，也不要生成 Steps、Timeline、复杂图表区、Tabs、弹窗或长数据映射。\n"
            )
        if is_single_module_retry and retry_module_page_variant == "studio":
            system_prompt += (
                "60. 当前模块 page_variant=studio：请体现运营驾驶舱/执行中台页特征，并围绕当前主题在以下结构中至少择一重构：`标题区 + 阶段摘要带 + 主结果表`、"
                " `标题区 + 关键节点摘要 + 左右双区块`、`标题区 + 筛选条 + 执行列表 + 结果回写区`；"
                " 不要退化成通用“统计分析/分析中心”页，也不要生成纯图表看板、空摘要卡、下载弹窗、Tabs、时间轴、复杂图表库或长篇说明。"
                " 若当前模块其实是 `StatisticsPage.tsx`，请直接围绕当前模块标题、筛选提示和样例行里的真实执行字段组织页面，"
                " 至少落下阶段/责任人/结果摘要中的多个真实锚点，不要只展示产品名和泛化统计口径。\n"
            )
...
"""
        ...
        if is_assets_module_retry:
            system_prompt += (
                ...
                "64. 若你正想输出另一类最新错误实现：`const records = [`，"
                " 并在数组项里复用 `id/topic/role/status/tag/updateTime` 这一套通用档案字段，"
        ...
        if is_workflow_module_retry:
            system_prompt += (
                ...
                "65. 若你正想输出另一类最新错误实现：`const thStyle = ...`、`const tdStyle = ...`、`const records = [`，"
...
"""
        ...
        if is_monitor_module_retry:
            system_prompt += (
                "\n"
                "59. 当前回补页面是 `MonitorPage.tsx`。注意：页面主题必须完全以当前模块画像为准；不要写通用“监控中心”“预警工作台”或历史项目监控口径。\n"
                "60. `MonitorPage.tsx` 请压缩成“1 个筛选输入 + 1 个主操作 + 1 张结果表/列表”；直接复用当前模块标题、主操作、筛选提示和任务样例数据，不要输出摘要卡矩阵、Tabs、弹窗或长篇说明。\n"
                "61. `MonitorPage.tsx` 不要使用 `useState`、`Card`、`Input`、`Button`、`Space`、`Tag`、`Table`、`SearchOutlined`、`PlusOutlined` 这一整套重型 antd 监控页实现；请改成无 hooks 的普通函数组件，优先使用原生 `<input>` + 原生 `<table>`。\n"
                "62. 若你正想输出类似这类最新错误实现：`import { useState } from 'react'`、`import { Card, Input, Button, Space, Tag, Table } from 'antd'`、`import { SearchOutlined, PlusOutlined } from '@ant-design/icons'`、`const [searchText, setSearchText] = useState('')`，那么这整种写法本轮一律判失败；页面主标题和结果表都必须逐字围绕当前模块标题与样例数据组织。\n"
            )
        if is_settings_module_retry:
            system_prompt += (
                "\n"
                "59. 当前回补页面是 `SettingsPage.tsx`。注意：页面主题必须完全以当前模块画像为准；不要写通用“系统设置中心”“平台参数配置台”或历史项目口径。\n"
                "60. `SettingsPage.tsx` 请压缩成“1 个检索输入 + 1 个保存按钮 + 1 张设置表/列表”；直接复用当前模块标题、主操作、筛选提示和任务样例数据，不要输出复杂表单分组、Tabs、抽屉、弹窗或长篇说明。\n"
                "61. `SettingsPage.tsx` 不要使用 `React.FC`、`Typography`、`Input`、`Button`、`Tag`、`Space`、`SearchOutlined` 这一整套重型 antd 设置页实现；请改成无 hooks 的普通函数组件，优先使用原生 `<input>` + 原生 `<table>`。\n"
                "62. 若你正想输出类似这类最新错误实现：`import React from 'react'`、`import { Input, Button, Tag, Space, Typography } from 'antd'`、`import { SearchOutlined } from '@ant-design/icons'`、`const { Title, Text } = Typography`、`const thStyle: React.CSSProperties = ...`，那么这整种写法本轮一律判失败；页面主标题和设置表都必须逐字围绕当前模块标题与样例数据组织。\n"
                "63. 若你正想输出另一类最新错误实现：`<div style={{ padding: 24, background: '#f0fdf9', minHeight: '100vh' }}>`、`const cellStyle = { border: '1px solid #bbf7d0', ... }`、`<h1 style={{ color: '#14532d', ... }}>` 这种浅绿色通用设置轻壳，那么同样一律判失败；必须改回当前模块标题逐字命中的真实设置业务页。\n"
            )
        if is_reports_module_retry:
            system_prompt += (
                ...
                "64. 若你正想输出另一类最新错误实现：`const pageStyle: React.CSSProperties = ...`、`const headerStyle: React.CSSProperties = ...`、`borderBottom: '2px solid #16a34a'`，"
                " 再配合 `<div style={{ padding: 20, background: '#f0fdf9', minHeight: '100vh' }}>` 这类浅绿色报表轻壳，那么同样一律判失败；"
                " `ReportsPage.tsx` 必须改回当前模块标题逐字命中的真实报表页。\n"
            )
        if is_statistics_module_retry:
            system_prompt += (
                ...
                "67. 若你正想输出 `<div style={{ padding: 24, background: '#f0fdf9', minHeight: '100vh' }}>`、`fontFamily:`、"
                " 并直接展示 `APP_PROFILE.product_name` 的浅绿色全页轻壳，那么同样一律判失败；必须改回当前模块标题逐字命中的真实统计页。\n"
                "68. 若你正想输出 `import React from 'react';`、`style={styles.container}`、`style={styles.title}`、`style={styles.subtitle}`，"
                " 再把主标题写成“数据分析与看板”并直接展示 `APP_PROFILE.productName`，那么同样一律判失败；必须整体改写成当前模块标题逐字命中的真实统计页。\n"
                "69. 若你正想输出 `import React from 'react';` + `export default function StatisticsPage()`，"
                " 再配合 `<div style={{ padding: '24px' }}>`、`<h1 style={{ fontSize: 24, fontWeight: 600 ... }}>`"
                " 和只展示 `APP_PROFILE.product_name` 的极简标题壳，那么同样一律判失败；必须整体改写。\n"
                "70. 若你正想输出 `const rows = [`，"
                " 并在数组项里复用“协同任务看板周报 / 协同任务看板月报 / 专题分析 / 趋势稳定 / 需持续跟踪”这类泛化分析描述，"
                " 再把产品名或版本号塞进 rows 正文，那么同样一律判失败；必须整体改写成当前模块标题逐字命中的真实统计页。\n"
            )
        ...
        if is_app_retry:
            user_prompt += (
                ...
                " 不要再回到 `useMemo/useLocation/useNavigate/NavLink/Tabs/TabsProps/Outlet`、"
                " `import { Routes, Route, Navigate, Link } from 'react-router-dom'`，"
                " 或 `Row, Col, Card, Button, Space` 这一类重型顶层壳。"
            )
        if is_single_dashboard_batch:
            user_prompt += (
        ...
        if is_monitor_module_retry:
            user_prompt += (
                " 当前是 MonitorPage.tsx 回补，但业务主题以当前模块画像为准；不要写通用监控中心、预警工作台或其他历史项目标题。"
                " 不要再返回 `import { useState } from 'react'`、`import { Card, Input, Button, Space, Tag, Table } from 'antd'`、"
                " `import { SearchOutlined, PlusOutlined } from '@ant-design/icons'` 或 `const [searchText, setSearchText] = useState('')`"
                " 这种通用重型监控页模板；"
                " 请改成无 hooks 的普通函数组件，使用原生 input + 主操作 + 原生 table。"
            )
        if is_settings_module_retry:
            user_prompt += (
                " 当前是 SettingsPage.tsx 回补，但业务主题以当前模块画像为准；不要写通用系统设置中心、平台参数配置台或其他历史项目标题。"
                " 不要再返回 `import React from 'react'`、`import { Input, Button, Tag, Space, Typography } from 'antd'`、"
                " `import { SearchOutlined } from '@ant-design/icons'`、`const { Title, Text } = Typography`"
                " 或 `const thStyle: React.CSSProperties = ...` 这种通用重型设置页模板；"
                " 也不要再返回 `<div style={{ padding: 24, background: '#f0fdf9', minHeight: '100vh' }}>`、"
                " `const cellStyle = { border: '1px solid #bbf7d0', ... }`、`<h1 style={{ color: '#14532d', ... }}>`"
                " 这种浅绿色通用设置轻壳；"
                " 请改成无 hooks 的普通函数组件，使用原生 input + 保存按钮 + 原生 table。"
            )
        if is_reports_module_retry:
            user_prompt += (
                ...
                " 也不要再返回 `const pageStyle: React.CSSProperties = ...`、`const headerStyle: React.CSSProperties = ...`、"
                " `borderBottom: '2px solid #16a34a'` 配合 `<div style={{ padding: 20, background: '#f0fdf9', minHeight: '100vh' }}>`"
                " 这种浅绿色报表轻壳页。"
            )
        if is_statistics_module_retry:
            user_prompt += (
                ...
                " 也不要再返回 `<div style={{ padding: 24, background: '#f0fdf9', minHeight: '100vh' }}>`、"
                " `fontFamily:` 并直接展示 `APP_PROFILE.product_name` 的浅绿色全页轻壳。"
                " 也不要再返回 `import React from 'react'` + `style={styles.container}` + `style={styles.title}` + `style={styles.subtitle}`"
                " 并把主标题写成“数据分析与看板”的通用数据看板壳。"
                " 也不要再返回 `export default function StatisticsPage()` + `<div style={{ padding: '24px' }}>`"
                " 或 `const rows = [` 并在数组项里复用“协同任务看板周报 / 协同任务看板月报 / 专题分析 / 趋势稳定 / 需持续跟踪”这类泛化分析描述。"
            )
        ...
        if is_single_app_batch:
            ...
            max_tokens_override = 2600 if (
                ...
                or is_monitor_module_retry
                or is_settings_module_retry
...
8. `required_manual_modules` 是系统固定必选章节，你无需改动章节存在性；你需要从 `optional_manual_modules` 备选池中挑选 4 到 7 个最适合当前产品的模块 key，填入 `selected_optional_modules`，让不同产品的说明书扩展章节可以有变化。
9. `selected_optional_modules` 中的 key 必须来自 `optional_manual_modules`，且优先覆盖产品、数据、研发、实施、测试、运维等不同维度，避免每次都选择同一组模块。
10. 必须根据 `variation_seed` 主动变化章节顺序、子标题命名和段落展开方式，禁止复用固定排版骨架。
11. 必须以企业身份撰写，用于正式陈述软件产品设计、功能结构、页面用途、业务流程和技术实现事实，不得写成面向终端用户的营销文案、培训话术或聊天式说明。
12. 页面标题与图片已在文档结构中给出，正文和页面说明里不要再使用“该截图展示了”“截图中重点可见”“下图所示”“从截图可以看出”等围绕截图本身的提示语，应直接进入功能陈述。
13. 应优先使用“本软件提供……”“系统实现了……”“该功能用于……”“该页面支持……”等企业陈述式表达，避免“你可以”“用户只需”“建议先点击”“通过截图可见”等指导式或观察式措辞。
14. 该文档属于企业提交给版权局了解项目真实开发情况的软件产品说明书/操作手册，不是模型给团队出的研发建议书、测试建议书、部署建议书、优化迭代建议书或验收建议书。
15. 不得出现“建议在研发阶段……”“验收时应重点核对……”“建议围绕……开展培训”“后续迭代建议”“功能测试建议”“研发测试与验收建议”这类面向团队的建议式标题或正文；相关内容如确需体现，必须改写为当前软件已经采用的设计、已有的功能机制、既定的检查项、既有的角色分工或正式交付内容。
16. 必须始终以当前产品自身为叙述主体，围绕本产品当前页面、模块、字段、角色、流程、截图、导出物和技术实现做事实性描述，不得把模型自己写成评审者、顾问、实施教练或测试负责人。
...
"""
        user_payload = {
            ...
            "variation_seed": profile.get("variation_seed") or build_variation_seed(product_name, profile.get("design_seed", "")),
        }
        messages = [
            {"role": "system", "content": system_prompt},
    ...
    async def generate_manual_markdown(
        self,
        *,
        product_name: str,
        version: str,
        ...
        """Generate a full software manual as Markdown with free layout."""
        variation_seed = str(profile.get("variation_seed") or "").strip() or build_variation_seed(
            task_id, product_name, profile.get("design_seed", ""), profile.get("keyword", "")
        )
        manual_retry_attempt = int(profile.get("manual_retry_attempt") or 0)
        module_profiles = [
            {
                ...
                "highlights": list(module.get("highlights", [])[:4]),
                "table_headers": list(module.get("table_headers", [])[:8]),
            }
            for module in profile.get("modules", [])[:10]
        ]
        screenshot_briefs = [
            {
        ...
        system_prompt = """你是一位中文软件说明书撰写专家。请为当前产品从零撰写一份完整的软件说明书 Markdown 正文。
硬性要求：
1. 输出 JSON：{"manual_markdown":"...", "selected_optional_modules":["key1","key2",...]}，其中 manual_markdown 为完整 Markdown 正文。
2. 必须覆盖 required_manual_modules 中的全部必选章节，但你可以自主决定章节顺序、子标题命名、段落组织、列表/表格形式；禁止套用固定后台说明书骨架。
3. 从 optional_manual_modules 中选择 4~7 个扩展模块 key 写入 selected_optional_modules，并在正文中输出对应章节。不同产品应根据自身特征选择不同的扩展模块组合，禁止每次都选同一组。
4. variation_seed 表示本次排版的唯一性约束：必须依据它主动拉开与历史任务不同的章节顺序、标题措辞和信息展开方式，禁止只替换产品名。
5. 软件使用说明章节中，每个主要页面必须插入截图占位符 [[SCREENSHOT:页面标题]]；系统架构图位置使用 [[SCREENSHOT:ARCH]]。
6. 第一行使用 `# 产品名称` 作为封面标题；版本号由系统追加，不要在正文重复写封面版式。
7. 必须以企业身份撰写软著申报材料，不得出现模型名称、AI 自动生成、截图元叙述、面向团队的建议式标题。
8. 技术名保留英文原名，如 FastAPI、React、TypeScript、PostgreSQL。
9. 页面说明应直接陈述功能用途，不得写“该截图展示了”“下图所示”等表达。
10. 正文必须充分展开，保证成品说明书可达 20 页以上，不得用重复段落凑字数。
11. `required_manual_modules` 为系统固定必选章节（必须全部出现，不可缺少任何一个）；`optional_manual_modules` 为选做章节池（你必须从中自主挑选 4~7 个 key 写入 selected_optional_modules 并撰写对应章节）。必选模块与选做模块的界限不可混淆：必选模块每个都必须出现，选做模块只需出现你选中的那些。
12. 禁止套用固定后台说明书排版骨架：章节顺序、编号体系（如 1.1 / 一、）、标题层级、列表/表格组织方式必须随 variation_seed 与 manual_retry_attempt 主动变化。
13. 每份说明书的排版风格必须全新设计：编号体系（数字编号、中文编号、混合编号、无编号）、标题层级（二级为主、三级为主、混合层级）、段落组织（先叙述后列表、先表格后说明、先图后文字等）、信息展开方式必须随 variation_seed 主动变化，禁止所有说明书都用同一套“二级标题 + 三级子标题 + 无序列表”的固定骨架。
14. manual_retry_attempt 递增时必须进一步拉开排版差异，不仅是内容变化，还要在章节顺序、编号风格和标题措辞上明显不同。
"""
        user_payload = {
            "product_name": product_name,
            "version": version,
    ...
    async def generate_architecture_diagram_spec(
        self,
        *,
        product_name: str,
        profile: dict,
        ...
        """Generate a project-specific architecture diagram spec for rendering."""
        variation_seed = build_variation_seed(task_id, product_name, profile.get("design_seed", ""), "architecture")
        modules = [
            {
                "title": module.get("title", ""),
                "route": module.get("route", ""),
        ...
        system_prompt = """你是企业软件架构图设计专家。请基于当前项目画像，设计一张全新的系统架构图规格，禁止复用固定五层盒子模板或只改 label。
输出 JSON：
{
  "title": "产品名称系统架构图",
  "caption": "图1：...",
  "layout_style": "layered_left_right|hub_spoke|pipeline|matrix|radial|modular_gallery",
  "nodes": [
    {"title":"节点标题","body":"节点说明","x":5,"y":12,"w":28,"h":18,"kind":"primary|secondary|accent"}
  ],
  "arrows": [{"from_index":0,"to_index":1}]
}
要求：
1. nodes 数量 4~7，x/y/w/h 使用 0~100 的相对坐标；布局必须体现当前 project_dna.architecture_style 与模块签名。
2. 节点标题与说明必须来自当前模块/角色/业务对象，不得使用通用“五层架构”套话。
3. variation_seed 不同则布局拓扑、节点命名和连线路径必须明显不同。
4. 全部中文，caption 中产品名与“系统架构图”之间不要多余空格。
5. arrows 表示有向依赖或数据流：连线必须连接节点矩形边框（edge anchor），禁止把箭头端点落在节点中心或节点文字区域内部；箭头端点必须明确停在矩形边框线上，不得与框内文字重叠。
6. 优先使用正交折线或清晰的边到边直线；节点之间应留出间距，避免连线穿过节点正文；箭头起止点应与框内文字保持至少 8px 间距。
7. from_index / to_index 仅引用 nodes 数组下标，不要输出超出范围的索引。
"""
        user_payload = {
            "product_name": product_name,
            "variation_seed": variation_seed,
