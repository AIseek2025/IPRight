...
38. 如果 `experience_blueprint.navigation_variant` 为 `indexed`，或 `visual_profile.chrome_treatment` 为 `indexed_topbar`，`App.tsx` 必须实现顶部索引导航、顶部切换条或顶部主导航，并可配合右侧摘要栏/概览区；严禁退回 `Layout.Sider`、左侧深色竖向导航、`Menu mode="inline"`、`theme="dark"` 这类通用后台壳层。
39. 如果 `experience_blueprint.shell_layout_hint` 明确要求“顶部索引区 + 右侧摘要栏 + 分析工作台”“顶部导航 + 内容画布”“分段导航 + 主工作区”之类结构，必须按该结构组织首页和壳层；不得仅使用 Ant Design 默认 `Layout + Sider + Menu` 套板。
